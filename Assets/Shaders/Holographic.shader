Shader "Oculus/Holographic"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert (appdata v)
            {
				v.vertex *= 1 + abs(sin(_Time.b * 5)/10);
                v2f o;
            ...
            fixed4 frag (v2f i) : SV_Target
			{
				...
				fixed4 rimColor = _RimColor * pow (rim,  sin(_Time.b) + _RimPower * 1.0f );

                float glow = frac(dirVertex * _GlowTiling * pow(1 - i.uv.y,2) - _Time.x * _GlowSpeed);


				fixed4 col = rimColor + _MainColor * _MainColor.a;
